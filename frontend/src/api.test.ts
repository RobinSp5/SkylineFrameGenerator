...
describe("createJob", () => {
  it("posts the spec and returns the id", async () => {
    ...
    const res = await createJob({ center_lat: 50, center_lon: 8, side_m: 1000, rotation_deg: 0, plate_size_mm: 100, plate_thickness_mm: 3, mode: "simple", z_exaggeration: 1.5, lod2: true, terrain: false, terrain_exaggeration: 1, trees: true, print_optimized: true, multicolor: false });
  ...
  it("passes the abort signal to fetch", async () => {
    ...
    await createJob({ center_lat: 50, center_lon: 8, side_m: 1000, rotation_deg: 0, plate_size_mm: 100, plate_thickness_mm: 3, mode: "simple", z_exaggeration: 1.5, lod2: true, terrain: false, terrain_exaggeration: 1, trees: true, print_optimized: true, multicolor: false }, controller.signal);
