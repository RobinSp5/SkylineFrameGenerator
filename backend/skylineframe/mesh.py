...
def terrain_solid(hf: Heightfield, thickness_mm: float) -> m3d.Manifold:
    ...
    """
    return grid_solid(hf, hf.z_mm, -thickness_mm)
...
def _sockel_on(blocks: list[Prism], hf: Heightfield, spec: FrameSpec, sink: float) -> m3d.Manifold | None:
    ...
    layers = [
        _columns(polys, hf, spec, height) ^ grid_solid(hf, hf.z_mm + height, hf.z_mm - sink)
...
def _recess_on(
    ...
    below = hf.z_mm - depth
    cutter = columns ^ grid_solid(hf, float(hf.z_mm.max()) + EPS, below)
    inlay = columns ^ grid_solid(hf, hf.z_mm, below)
    return cutter, inlay
