...
Datum: 2026-09-24
Status: freigegeben (Teil 1 im Chat bestätigt, Rest auf Zuruf „zieh das flott durch“)
...
Die Trennung „druckbar → eigener Körper / sonst → Block“ entfällt. Jeder Footprint nach `assign_parts` mit Fläche ≥ `TINY_FOOTPRINT_MM2 = 0.1` mm² (Druckraum) wird ein Gebäude in `Prepared.buildings`. Kleinere (Schuppen ~5 m² bei 1:15 000) tragen nur noch zum Sockel bei.
...
`MIN_LINE_MM = 0.4` (eine Düsenbahn). Ein Footprint, der eine Erosion um `MIN_LINE_MM / 2` nicht überlebt, wird mit Gehrungs-Puffer (`join_style="mitre"`) um den kleinsten Radius `r ∈ (0, MIN_LINE_MM / 2]` wachsen gelassen, nach dem er sie überlebt (Bisektion oder `shapely.maximum_inscribed_circle`, Shapely 2.1.2 ist installiert). Höchstens 0,2 mm pro Seite, die Lage bleibt erhalten. Das Verbreitern passiert nach `assign_parts` und **vor** `resolve_roof`, damit das Dach-Rechteck zum verbreiterten Grundriss passt.
...
Ein OSM-Gebäude (nicht LoD2, kein Part) ohne `roof:shape` bekommt `RoofSpec(shape="gabled")`, wenn `kind` in `{house, detached, semidetached_house, bungalow}` liegt oder `kind == "residential"` mit Grundfläche < 200 m². Die Rechteck-Regel (`ROOF_RECT_RATIO`) in `resolve_roof` entscheidet wie bei getaggten Dächern, ob es gebaut wird. Dachhöhe aus `default_roof_height_m`. Die Zuweisung steht in einer eigenen reinen Funktion `default_roof(kind, area_m2) -> RoofSpec | None` und läuft in `prepare` direkt nach `estimate_missing_heights`, also vor dem `spec.roofs`-Abschalter, sodass `--no-roofs` auch Standarddächer abschaltet.
