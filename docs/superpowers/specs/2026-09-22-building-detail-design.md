...
2. **Höhen auffüllen**: Gebäude ohne Höhenangabe erhalten die Schätzung aus §5, berechnet auf der projizierten, aber noch nicht auf das Quadrat geschnittenen Grundfläche (die Schätzung ist eine Eigenschaft des Gebäudes, nicht des Ausschnitts).
...
4. **Blöcke bilden**: Alle Grundrisse (Umrisse ohne Teile, Teile, Restbereiche) werden 2D vereinigt mit einem Close um `MIN_FEATURE_MM / 2 / scale` (dilate → erode, runde Verbindungen, Bogen-Sehnen anschließend vereinfacht). Im Full-Modus werden die Straßenkorridore (gepufferte Straßen) vor dem Close aus der Eingabe entfernt, und die Wasserflächen abzüglich der Gebäudegrundrisse werden nach dem Close vom Ergebnis abgezogen, damit das Close weder eine Straße noch einen Kanal überbrückt, Gebäude in oder am Wasser aber ihren Block behalten; einzeln druckbare Gebäude behalten wie bisher Vorrang vor Straßen. Jede resultierende Fläche ist ein **Block**. Blockhöhe = 25. Perzentil der Traufhöhen der enthaltenen Grundrisse (flächengewichtet). Die Mindesthöhe `min_building_height_mm` wird erst bei der Skalierung angewendet (wie bei Gebäuden), nicht als Meter-Untergrenze in `prepare`, sonst würde `z_exaggeration` doppelt wirken.
...
- Einzeln druckbare Gebäude: Prisma von `max(min_height, 0)` bis Traufe, plus Dach. Teile mit `min_height > 0` beginnen in der Luft; da sie in der Praxis auf niedrigeren Teilen desselben Gebäudes stehen, entsteht nach der 3D-Vereinigung ein zusammenhängender Körper. Freischwebende Teile werden bis 0 verlängert (Sicherheitsregel): `min_height` gilt nur, wenn die Vereinigung aller Körper desselben Umrisses, die die Unterkante des Teils tragen, mindestens 25 % der Teil-Grundfläche überdeckt (`SUPPORT_FRACTION = 0.25`); ein Punktkontakt ist keine Auflage.
